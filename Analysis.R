...
# This analysis considers how series length, episode type, and the variety of writers and directors either positively or 
# negatively impact the amount of UK Viewers and/or the Appreciation Index for the sci-fi show "Doctor Who".
...
Scatter <- function(xaes, yaes, na.rm = T) {
  xaes <- enquo (xaes)
  yaes <- enquo(yaes)
  ggplot(dw, aes(!!xaes,!!yaes)) +
...
ScatterSer <- function(xaes, yaes, na.rm = T) {
  xaes <- enquo (xaes)
  yaes <- enquo(yaes)
  ggplot(dwSer, aes(!!xaes,!!yaes)T) +
  scale_size(range = c(.1, 10)) +
...
ScatterSp <- function(xaes, yaes, na.rm = T) {
  xaes <- enquo (xaes)
  yaes <- enquo(yaes)
  ggplot(dwSp, aes(!!xaes,!!yaes)) +
  scale_size(range = c(.1, 10)) +
...
# Despite the Appreciation Index being relatively stable for each series length, with each reduction in series length, 
# the average Appreciation Index decreases (e.g. 86 for 13 episodes, compared to 78 for those with 6-8 episodes), 
# indicating that audiences are more likely to view a longer series, with a greater variety of writers, more favourably.

# This is also reflected by the Median UK Viewers for each series length, with 13 Episodes per Series being 7.63 million, 
# which is over double that of a series that has 8 Episodes (3.64) million.

# Furthermore, with each reduction in series length, the correlation between Appreciation Index and UK Viewers becomes stronger. 
# Additionally, the ranges for each also become lower, highlighting the benefit of longer series for both views and ratings.
...
# Series One to Five benefited from essentially the same episode story types, broadcast pattern, 
# and a 13-episode series length. This allowed for viewer habits and expectations to develop, until both views and ratings peaked 
# with Series Four.

# Thereafter, starting with Series Six, changes to the story type pattern started to cause a negative trend for both views
# and ratings. 
# Interestingly, once the series length began to decrease, the highest Appreciation Index achieved for 12 Episodes
# was 85 - which decreased to 83 for 10 episodes, and 80, when the series length had 6-8 episodes. 
# Conversely, for 13 episodes, the Appreciation Index reached 91.

# For Special Episodes, there was a similar pattern: when the regular series length was 13 episodes, 
# the Appreciation Index reached a maximum of 89 - which decreased to 82 for 12 episodes, and 80 for 10 episodes. 
# The 2023 Specials, on the other hand, bucked the trend, by reaching a high of 85, and having views that were of similar levels
# to those of a special for a series with 12 episodes.
...
# With regards to the Directors, any correlation between the decline in views and ratings are hard to determine, 
# due to to only 5 (Hettie MacDonald, Douglas Mackinnon, Peter Hoar, Charles Palmer, and Jamie Magnus Stone) of the 66 directors 
# directing episodes from differing series lengths.
...
# Disruptions (e.g. the 2008-10 Specials occurring between the finale for Series Four and the start of Series Five, or midway 
# series breaks during Series Six and Seven), as well as changes (e.g. reduced series length, and varied broadcasting date 
# starts), to the series pattern all contributed to the decline in both views and ratings - hence why views skew negatively, with 
# each reduction in series length.

# The only that pattern that remained across each series, regardless of Showrunner, was that the Showrunner will write the first 
# and last episode in a series/block of episodes in a series. However, with reduced number of episodes per series, the number of 
# Showrunner written episodes dwarves the number of non-showrunner written episodes.

# For example, with Series One, Russell T Davies wrote 8 of the 13 episodes (61.5%), dropping to 5 of the 13 episodes (38-38.5%) 
# for the rest of his initial tenure (Series Two to Four). However, when he returned as Showrunner, for Series Fourteen, he wrote
# 5 of the 8 (75%) episodes, meaning that only 12% (1 episode) of the series was not written by either the current or a past 
# Showrunner. Interestingly, Series Fifteen had an inverse to Series Two to Four, where 38% (3 of the 8 episodes) of the series 
# was not written or co-written by the Showrunner. 
# The consequence of this indicates the reduced ability to develop talent to showrun a new era after Russell T Davies leaves, as
# well as the show overly relying on the showrunner for writing.
                                                                                                                                                                                                                                                                           
# CONCLUSION

# Variety is the spice of life, but for a show like "Doctor Who" to thrive, it needs to develop a framework that compliments the 
# restrictions of a reduced series length, rather than attempt to replicate the format of a series with 13 episodes. Moreover, 
